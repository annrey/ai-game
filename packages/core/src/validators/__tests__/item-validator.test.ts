...
import type { Item } from '@openclaw/shared-types';
import type { ItemGenerationContext, ValidationResult } from '@openclaw/shared-types';
...
describe('ItemValidator', () => {
  ...
  describe('validateItem - 物品验证', () => {
    ...
    it('应拒绝无效类型的物品', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('物品类型必须是 drink, food, misc, service, weapon, armor, consumable, quest 之一');
    });
