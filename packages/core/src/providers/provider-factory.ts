...
import type { AIProvider, ProviderConfig } from '@openclaw/shared-types';
import type { AgentRole } from '@openclaw/shared-types';
...
export class ProviderFactory {
  ...
  /** 注册第三方 Provider 扩展 */
  registerProviderExtension(extension: ProviderExtension): void {
    const extId = extension.manifest.id;
    if (this.providerExtensions.has(extId)) {
      console.warn(`Provider 扩展 ${extId} 已注册过。`);
      return;
    }
    this.providerExtensions.set(extId, extension);
  }
  ...
  getByType(type: ProviderConfig['type']): AIProvider {
    ...
    if (!provider) {
      // 尝试在扩展中查找
      const extension = this.providerExtensions.get(type);
      if (extension) {
        // 创建 Provider 实例并缓存
        const extConfig: ProviderConfig = {
          type: type,
          // 用户可以在 ProviderFactoryConfig 中通过扩展名称传递自定义配置
          ...(this.config as any)[type],
        };
        provider = extension.createProvider(extConfig);
        this.providers.set(type, provider);
        return provider;
      }
      throw new Error(`Provider "${type}" 不可用。请检查配置或确保相关插件已加载。`);
    }
    return provider;
  }
  ...
  /** 获取所有已实例化的和可通过扩展实例化的 Provider */
  private getAllProviders(): Map<string, AIProvider> {
    const all = new Map(this.providers);
    for (const [id, ext] of this.providerExtensions) {
      if (!all.has(id)) {
        try {
  ...
  async checkAvailability(): Promise<Record<string, boolean>> {
    ...
    for (const [name, provider] of this.getAllProviders()) {
